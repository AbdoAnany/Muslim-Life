...
class PrayerTimesStorage {
  ...
  static Future<void> savePrayerTimes(PrayerTimesModel prayerTimes) async {
    ...
    prayerTimes.data!.forEach((element) {
      ...
      timingsElement.forEach((element) async {
        ...
        if(originalDate.isAfter(DateTime.now())) {

       await zonedScheduleNotification(id:x,
         title: '${element?.arabicName!}',
         body:element!.englishName!.length > 7 ? "حان الان  ${element.arabicName!}":"حان الان موعد أذان ${element.arabicName!}",
  ...
  static Future<PrayerTimesModel?> getPrayerTimes() async {
    ...
    if(lastSync.isEmpty){
      ...
      if(!isSameMonth(DateTime.parse(lastSync)))
     {
       prefs.setString(key,'');
       jsonString='';
     }
